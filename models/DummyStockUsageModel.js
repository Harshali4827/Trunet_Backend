...
stockUsageSchema.pre("save", function (next) {
  switch (this.usageType) {
    ...
    case "Control Room":
      ...
      case "Damage": 
      if (!this.warehouse) {
        return next(new Error("Warehouse is required for damage usage type"));
      }
      break;
...
stockUsageSchema.statics.createStockUsage = async function (usageData) {
  try {
    ...
    const stockCenter = usageData.usageType === 'Damaged' ? usageData.warehouse : usageData.center;
...
stockUsageSchema.methods.reserveStockForDamage = async function () {
  ...
  try {
    ...
    for (let item of this.items) {
      ...
      const damageCenter = this.warehouse || this.center;
