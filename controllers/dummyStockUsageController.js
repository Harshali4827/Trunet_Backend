...
export const createStockUsage = async (req, res) => {
  try {
    ...
    if (usageType === "Damage" && !warehouse) {
      return res.status(400).json({
        success: false,
        message: "Warehouse is required for damaged usage type",
      });
    }
    ...
    for (let item of items) {
      ...
      // const stockCenter = usageType === "Damage" ? warehouse : center;
      // const validatedItem = await validateAndCalculateStock(item, stockCenter);
    ...
    const populatedStockUsage = await StockUsage.findById(stockUsage._id)
      ...
      .populate("warehouse", "name centerType")
...
const addToFaultyStockCollection = async (stockUsage) => {
  try {
    ...
    for (let item of stockUsage.items) {
      const product = await Product.findById(item.product);
      const fromCenter = await Center.findById(stockUsage.center).populate('reseller');
      const toCenter = await Center.findById(stockUsage.warehouse).populate('reseller');
      ...
      if (!fromCenter) {
        console.log(`From Center not found: ${stockUsage.center}`);
        continue;
      }

      if (!toCenter) {
        console.log(`To Center (Warehouse) not found: ${stockUsage.warehouse}`);
        continue;
      }

      const fromReseller = fromCenter.reseller;
      const toReseller = toCenter.reseller;
      
      if (!fromReseller) {
        console.log(`No reseller found for fromCenter: ${fromCenter.centerName}`);
        continue;
      }

      if (!toReseller) {
        console.log(`No reseller found for toCenter: ${toCenter.centerName}`);
        continue;
      }
      ...
      const faultyStockData = {
        ...
        fromCenter: stockUsage.center,
        toCenter: stockUsage.warehouse,
        fromReseller: fromReseller._id,
        toReseller: toReseller._id,
        product: item.product,
        quantity: item.quantity,
        serialNumbers: [],
        usageType: stockUsage.usageType,
      ...
      if (item.serialNumbers && item.serialNumbers.length > 0) {
        ...
        // For non-serialized products, create placeholder serial numbers
      ...
      // Check if faulty stock entry already exists
      ...
      // Update center stock to mark items as damaged
...
const validateUsageTypeFields = async (usageType, fields) => {
  const { customer, fromBuilding, toBuilding, fromControlRoom, warehouse } = fields;
  ...
  switch (usageType) {
    ...
    case "Damage":
      if (!warehouse) return "Warehouse is required for damaged usage type";
      if (!mongoose.Types.ObjectId.isValid(warehouse))
        return "Invalid warehouse ID";
      const warehouseExists = await Center.findById(warehouse);
      if (!warehouseExists) return "Warehouse not found";
      break;
...
const addUsageTypeSpecificFields = (stockUsageData, fields) => {
  ...
  switch (stockUsageData.usageType) {
    ...
    case "Damage":
      console.log("Setting warehouse for Damage:", warehouse); 
      stockUsageData.warehouse = warehouse;
      break;
  }
};
...
if (!centerStockUpdate) {
  ...
  console.log(`✓ CenterStock updated: '${serialNumber}' (consumed → available)`);
      ...
      const returnData = {
        ...
        warehouse:originalUsage.warehouse,
        remark: remark,
        returnedBy: returnedBy,
        status: "completed"
...
export const getAllFaultyStock = async (req, res) => {
  try {
    ...
    const faultyStockRecords = await FaultyStock.find(filter)
      .populate("fromCenter", "centerName centerType")
      .populate("toCenter", "centerName centerType")
      .populate("fromReseller", "name email")
      .populate("toReseller", "name email")
