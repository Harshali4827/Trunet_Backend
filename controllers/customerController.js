...
export const importCustomers = async (req, res) => {
  try {
    ...
    /* 🔹 Fetch all existing usernames ONCE (FAST for 1 lakh records) */
    const dbUsers = await Customer.find({}, "username");
    dbUsers.forEach(u => existingUsernames.add(u.username));

    const findCenterByName = async (centerName) => {
      const key = centerName.trim().toLowerCase();
      ...
      fs.createReadStream(filePath)
        ...
        .on("data", async (row) => {
          ...
          try {
            ...
            const centerName = row.center || row.center_title;

            if (!username || !mobile || !centerName) {
              failedCount++;
              errors.push({
                row: rowNumber,
                username: username || "N/A",
            ...
            if (processedUsernames.has(username) || existingUsernames.has(username)) {
              failedCount++;
              errors.push({
                row: rowNumber,
                username,
            ...
            const center = await findCenterByName(centerName);
            if (!center) {
              failedCount++;
              errors.push({
                row: rowNumber,
                username,
                error: `Center not found: ${centerName}`
              });
              return;
            }
