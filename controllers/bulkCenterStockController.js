 ...
 * centerIdentifier,productIdentifier,availableQuantity,serialNumbers
 * 
  ...
  try {
    ...
    for (let index = 0; index < results.length; index++) {
      ...
      try {
        ...
        // Parse available quantity (this will be used for both total and available)
        ...
        if (serialNumbersStr && serialNumbersStr.trim() !== '') {
          ...
          // Create serial number objects with proper structure
          serialNumbers = serialList.map(serial => ({
            serialNumber: serial,
            purchaseId: new mongoose.Types.ObjectId(),
            originalOutlet: centerInfo.id,
        ...
        if (stockEntry) {
          ...
          // Set totalQuantity equal to availableQuantity
          // Set inTransitQuantity and consumedQuantity to 0
          ...
          if (tracksSerialNumbers && serialNumbers.length > 0) {
            // Replace existing serials with new ones
            stockEntry.serialNumbers = serialNumbers;
          } else if (!tracksSerialNumbers) {
            // For non-serialized products, clear serials
...
/**
 * Download sample CSV template for center stock (simplified)
 */
export const downloadCenterStockSampleCSV = async (req, res) => {
  try {
    // Get some sample centers and products
