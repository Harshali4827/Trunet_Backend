  ...
  try {
    ...
    try {
      ...
      for (const productItem of products) {
        ...
        const resellerStockAfter = await addToResellerStock(
          resellerId, 
          productId, 
          quantity, 
          serialNumbers, 
        ...
        processedItem.resellerStockAfter = {
          totalQuantity: resellerStockAfter.totalQuantity,
          availableQuantity: resellerStockAfter.availableQuantity,
          consumedQuantity: resellerStockAfter.consumedQuantity,
          damagedQuantity: resellerStockAfter.damagedQuantity,
          repairQuantity: resellerStockAfter.repairQuantity,
          sourceBreakdown: resellerStockAfter.sourceBreakdown
        };

        processedProducts.push(processedItem);
      }

      const centerReturnRecord = new CenterReturn({
        date: returnDate,
        remark: remark || "",
        center: centerId,
        reseller: resellerId,
        products: processedProducts.map(item => ({
          ...
          quantity: item.quantity,
          serialNumbers: item.serialNumbers,
          centerStockBefore: item.centerStockBefore,
          centerStockAfter: item.centerStockAfter,
          resellerStockAfter: {
            totalQuantity: item.resellerStockAfter.totalQuantity,
            availableQuantity: item.resellerStockAfter.availableQuantity,
            consumedQuantity: item.resellerStockAfter.consumedQuantity
          }
        })),
        processedBy: userId,
        status: "completed",
      ...
      await centerReturnRecord.save();

      res.status(200).json({
        success: true,
        message: "Stock returned to reseller successfully",
        data: {
          returnId: centerReturnRecord._id,
          returnRecord: {
            date: returnDate,
            remark: remark || "",
            center: centerId,
...
async function addToResellerStock(
  ...
  try {
    ...
    if (!resellerStock) {
      resellerStock = new ResellerStock({
        ...
        sourceBreakdown: {
          ...
          centerReturnQuantity: 0,
          directPurchaseQuantity: 0,
          // Add these arrays to track which centers stock came from
    ...
    // Update source breakdown by center
    const centerReturnEntry = resellerStock.sourceBreakdown.centerReturnFromCenters.find(
      entry => entry.center.toString() === sourceCenter.toString()
    );
    
    if (centerReturnEntry) {
      centerReturnEntry.quantity += quantity;
    } else {
      resellerStock.sourceBreakdown.centerReturnFromCenters.push({
        center: sourceCenter,
        quantity: quantity
      });
    }
      ...
      for (const serialNumber of serialNumbers) {
        ...
        if (existingSerialIndex !== -1) {
          ...
          if (existingSerial.status === "consumed" || existingSerial.status === "damaged") {
            ...
            existingSerial.sourceCenter = sourceCenter; // Track which center it came from
            existingSerial.sourceReference = null; // Could be CenterReturn ID
            existingSerial.currentLocation = resellerId;
          }
          ...
          existingSerial.transferHistory.push({
            ...
            transferType: "center_to_reseller_return",
            sourceType: "center_return",
            sourceCenter: sourceCenter, // Add sourceCenter to transfer history
            referenceId: null,
            remark: `Returned from center: ${remark || "No remark"}`,
            transferredBy: userId
          });
          ...
          resellerStock.serialNumbers.push({
            ...
            status: "available",
            sourceType: "center_return",
            sourceCenter: sourceCenter, // Track which center it came from
            sourceReference: null, // Could be CenterReturn ID
            currentLocation: resellerId,
            transferHistory: [{
              fromCenter: sourceCenter,
              toReseller: resellerId,
