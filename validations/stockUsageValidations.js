...
export const validateCreateStockUsage = [
  ...
  body("connectionType")
    .optional()
    .isIn(["NC", "Convert", "Shifting", "Repair"]),
  ...
  body("items.*.serialNumbers.*").optional().isString().trim(),
...
export const validateDamageReturnApproval = [
  param("id").custom(isValidObjectId).withMessage("Invalid stock usage ID"),
  ...
  body("acceptedQuantities")
    ...
    .custom((value, { req }) => {
      if (req.body.status === "accepted" && (!value || value.length === 0)) {
        throw new Error(
          "Accepted quantities are required when status is accepted"
        );
      }
      return true;
    }),
